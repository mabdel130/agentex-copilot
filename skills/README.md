...
| **[setup-permissions](./setup-permissions/SKILL.md)** | Installs or removes a user-level `agentex` launcher that starts Copilot CLI with AgenTeX's narrow Playwright/Node allow rules and destructive-command denials in any project. Directory trust remains project-specific; Azure rules are opt-in. |
